...
# tr5_f.x=sapply(1:length(y.x),function(h)  {eval_p_mean(y.x[h],xx=xx.x[h,],tr,hp=hp.x[h])}) 
# tr5_f.y=sapply(1:length(y.y),function(h)  {eval_p_mean(y.y[h],xx=xx.y[h,],tr,hp=hp.y[h])}) 


# tr5_cindex = cbind(tr5_f.x,tr5_f.y)
tr5_cindex = sapply(1:nrow(to.cindex.info_added[[tr]]),
                    function(h) {  eval_p_cindex(y.x=y.x[h],xx.x=xx.x[h,],tr.x=tr,hp.x=hp.x[h],
                                                 y.y=y.y[h],xx.y=xx.y[h,],tr.y=tr,hp.y=hp.y[h]) })
# file --------------------------------------------------------------------
save(tr5_cindex,file=paste0("tr",tr,"_cindex.Rdata"))
